# Pull and calculate hunter statistics by state, for Graphic 1
#
# 15-Mar-2018 E Silverman
#
createDiaryDataSummary.fn<-function(tableName="dbo.DkGsEsts_ESilverman",interval=10) {
  ...
  out<- diaryData %>% filter(Season > max(Season)-interval) %>%
    group_by(St) %>%
     ...
     mutate(name = state.name[match(St,state.abb)], duck_bag = round(duckBagPH), duck_hunters = (100*round(duckHunters/100)), 
